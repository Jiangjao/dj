...
email.onfocus = function () {
    items[7].innerHTML = "请输入您邮箱的正确格式";
    items[7].style.color = "green";
};
email.onblur = function () {
    var reg = /^\w+@\w+.[a-zA-Z]{2,3}(.[a-zA-Z]{2,3})?$/;
    if (this.value == "") {
        items[7].innerHTML = "请您务必写入邮箱!";
        items[7].style.color = "red";
    } else {
        if (!reg.exec(email.value)) {
            items[7].innerHTML = "请输入邮箱正确格式";
            items[7].style.color = "red";
        } else {
            items[7].innerHTML = "格式正确";
            items[7].style.color = "green";
            test6 = true;
        }
    }
